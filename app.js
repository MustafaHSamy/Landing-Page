{
  ...
  document.addEventListener('click', (event) => {
    if (
      ...
      event.target.parentElement !== nav &&
  ...
  document.addEventListener('click', (event) => {
    if (event.target.classList.contains('nav--link')) {
      ...
      for (let i = 0; i < navLists.length; i++) {
        ...
        parent.classList.add('activeList');
      }
    }
  });
}
...
}
{
  ...
  sectionsContainer.addEventListener('click', (event) => {
    ...
    if (targetedElement.tagName === 'SPAN') {
      let paragraph = targetedElementParent.querySelectorAll('p');
      for (let i = 0; i <= paragraph.length; i++) {
        paragraph[i].classList.toggle('hidden-para');
        if (paragraph[i].classList.contains('hidden-para')) {
      ...
      !targetedElement.classList.contains('section') &&
      targetedElement !== sectionsContainer
    ) {
...
}
{
  ...
  const moveHomebtn = () => {
    if (
      ...
      !homebtn.classList.contains('home-link-hidden')
    ) {
...
}
{
  ...
  document.addEventListener('click', (event) => {
    if (
      event.target !== navBtn &&
      navMenu.classList.contains('nav-menu-shown')
    ) {
      toggleMenu();
    }
  });
